...
# 유형별로 골라 쓰는 방식은 폐기했다(ANL-04). 분류 결과를 기다려야 해서
# 병렬 실행이 깨지고, 회의록·보고서에도 금액이 나오는데 놓치기 때문이다.
class AnalyzerType(str, Enum):
    SUMMARY = "summary"
    CATEGORY = "category"
    EXTRACT = "extract"    # 액션아이템 · 결정사항 · 일정 (ANL-03)
    AMOUNT = "amount"      # 금액 항목 (AMT-01). 전용 모델·어댑터를 쓴다
...
# 낼 때 이중으로 더해진다. 합계 대조(AMT-03)가 틀리는 가장 흔한 원인이다.
...
# 들어가면 사용자가 도구를 신뢰하지 않게 되기 때문이다(TSK-03 자동 등록 금지).
