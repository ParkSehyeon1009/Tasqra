...
#      맞춘 값이었다.
#
#   왜 250 으로 올렸나 — **배포된 모델이 실제로 쓰는 길이가 그보다 길다.**
#   Ollama(q8_0)에 올린 sum-v6 을 실제 문서 25건으로 재니 중앙 175자에
#   꼬리가 242자였고, 200 을 넘긴 8건은 **내용이 전부 정확했다**(환각 0,
#   다국어 0, 파싱 100%). 상한이 200이면 멀쩡한 요약 32%를 스키마가 버린다.
#   250 으로 올리면 25건이 전부 통과한다.
#
#   ⚠️ 표본 25건이다. 교차검증에서는 307자도 나온 적이 있어 250이 전부를
#     잡지는 못한다. 상한은 「튀는 것을 막는 장치」이지 문체 강제 수단이 아니다.
#
#   ⚠️ output_schemas.SummaryOutput 의 max_length 도 **함께** 250 이어야 한다.
#     한쪽만 바꾸면 모델이 쓰는 길이와 검증이 어긋난다.
#     GroundedSummaryOutput 이 상속하므로 긴 문서 경로도 함께 따라간다.
#
#   ⚠️ 이 파일은 AgentLearning/src/prompts.py 와 문자 단위로 같아야 한다.
...
2~3문장, 공백 포함 250자 이내입니다. 정보가 적으면 더 적은 문장을 허용합니다.
...
출력: {"summary":"250자 이내 요약","evidence_ids":["기존 id"]}
"""
