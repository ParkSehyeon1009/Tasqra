...
def upload_document(file: UploadFile = File(...), access: ProjectAccess = Depends(get_project_editor_access), service: ExtractionService = Depends(get_extraction_service)):
    ...
    document = service.upload_and_extract(access.project.id, access.member.user_id, file.filename, content)
