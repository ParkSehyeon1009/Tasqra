...
class TaskSuggestionWriter:
    ...
    def write(self, *, project_id, document_id, source_text_revision,
        ...
        rows = [TaskSuggestion(project_id=project_id, document_id=document_id,
            ...
            actor_scope=item.actor_scope, statement_type=item.statement_type,
            task_kind=item.task_kind, modality=item.modality,
            recipient=item.recipient, relative_expression=item.relative_expression,
