...
export default function DocumentHeader({ document, onBack, onDownload, onDelete, canEdit, busy }) {
  ...
  return <header className='document-page-header'>
    <div><button className='back-button' onClick={onBack}>← 문서 목록</button><div className='document-title-line'><span className='detail-file-icon'>{document.file_type.toUpperCase()}</span><div><h1>{document.filename}</h1><p>{document.file_type.toUpperCase()} · 총 {document.page_count ?? 0}쪽{counts.length ? ' · ' : ''}{counts.map(item => item.label + ' ' + formatNumber(item.value) + '자').join(' · ')}</p></div></div></div>
    <div className='document-page-actions'><div className='document-header-statuses'><StatusPill status={documentStatus}/><StatusPill status={reviewStatus}/></div><button onClick={onDownload} disabled={busy}>원본 다운로드</button>{canEdit && <button className='danger-outline' onClick={onDelete} disabled={busy}>문서 삭제</button>}</div>
  </header>
}
