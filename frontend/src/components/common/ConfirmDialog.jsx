...
export default function ConfirmDialog({ open, title, message, confirmLabel = '확인', danger = false, confirmationText, onConfirm, onCancel }) {
  ...
  useLayoutEffect(() => {
    ...
    const confirmationInput = dialogRef.current?.querySelector('input')
  ...
  return <div className="dialog-backdrop" role="presentation" onMouseDown={cancel}><section className="confirm-dialog" ref={dialogRef} role="alertdialog" aria-modal="true" aria-labelledby={titleId} aria-describedby={messageId} onKeyDown={handleKeyDown} onMouseDown={event => event.stopPropagation()}>
    <h2 id={titleId}>{title}</h2><p id={messageId}>{message}</p>
    {confirmationText && <label className="confirm-dialog__verification"><span>계속하려면 <strong>{confirmationText}</strong>을(를) 입력하세요.</span><input value={typedText} onChange={event => setTypedText(event.target.value)} autoComplete="off"/></label>}
