...
import { analyzeDocument, deleteDocument, downloadDocumentSource, getDocument } from '../api/document'
...
export default function DocumentDetailPage({ user, onLogout, notify }) {
  ...
  return <div className="document-detail-page"><AppHeader user={user} onLogout={onLogout} notify={notify} project={projectQuery.data}/><div className="document-detail-shell"><DocumentHeader document={document} canEdit={canEdit} busy={deleteMutation.isPending || downloadMutation.isPending} onBack={() => navigate(`/projects/${projectId}/documents`)} onDownload={() => downloadMutation.mutate()} onDelete={() => setDeleteOpen(true)}/><nav className="document-detail-tabs">{TABS.map(([key, label]) => <button className={activeTab === key ? 'active' : ''} key={key} onClick={() => setParams({ tab: key })}>{label}{key === 'analysis' && document.analyses.length > 0 && <b>{document.analyses.length}</b>}</button>)}</nav><main className="document-tab-body">{activeTab === 'content' && <DocumentContentTab document={document}/>} {activeTab === 'review' && <DocumentReviewTab document={document} onOpenReview={() => navigate(`/projects/${projectId}/documents/${documentId}/review`)}/>} {activeTab === 'analysis' && <DocumentAnalysisTab document={document} canAnalyze={canEdit} analyzing={analyzeMutation.isPending} onAnalyze={() => analyzeMutation.mutate()}/>} {activeTab === 'history' && <DocumentHistoryTab projectId={projectId} document={document}/>}</main></div><ConfirmDialog open={deleteOpen} title="문서를 완전히 삭제하시겠습니까?" message="원본 파일, 추출 텍스트, OCR 수정 이력과 분석 결과가 모두 삭제되며 복구할 수 없습니다." confirmationText={document.filename} confirmLabel="문서 영구 삭제" danger onCancel={() => setDeleteOpen(false)} onConfirm={() => { setDeleteOpen(false); deleteMutation.mutate() }}/></div>
}
