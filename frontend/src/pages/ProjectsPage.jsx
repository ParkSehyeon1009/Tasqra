...
export default function ProjectsPage({ user, onLogout, notify }) {
  ...
  async function submit(event) {
    ...
    try {
      const project = await createMutation.mutateAsync(Object.fromEntries(new FormData(form)))
      form.reset(); setCreating(false); navigate(`/projects/${project.id}/documents`)
    } catch { /* mutation에서 공통 오류 알림 처리 */ }
  }
  ...
  return <div className="project-screen"><header className="global-header"><Logo/><div className="user-chip"><span>{user.name}</span><i>{user.name.slice(0, 1)}</i><button onClick={onLogout}>로그아웃</button></div></header>
    <main className="project-main"><div className="project-title"><div><h1>내 프로젝트</h1><p>문서를 올리고 팀과 함께 정리할 공간을 선택하세요.</p></div><button className="primary" onClick={() => setCreating(true)}>새 프로젝트</button></div>
      {creating && <form className="create-project" onSubmit={submit}><input name="name" placeholder="프로젝트 이름" required autoFocus/><input name="description" placeholder="프로젝트 설명 (선택)"/><button className="primary" disabled={createMutation.isPending}>프로젝트 만들기</button><button type="button" onClick={() => setCreating(false)}>취소</button></form>}
      {loading ? <LoadingState label="프로젝트를 불러오는 중..."/> : error ? <div className="error-state">{error.message}</div> : <div className="project-cards">{projects.map(project => <ProjectCard project={project} onClick={() => navigate(`/projects/${project.id}/documents`)} key={project.id}/>)}<button className="project-card project-card--new" onClick={() => setCreating(true)}><b>＋</b><strong>새 프로젝트 만들기</strong><span>문서를 모아둘 공간을 만듭니다.</span></button></div>}
      {!loading && !projects.length && <div className="empty-card"><b>＋</b><h2>첫 프로젝트를 만들어 보세요.</h2></div>}
    </main></div>
}
