...
export default function OcrReviewPage({ user, onLogout, notify }) {
  ...
  return <div className='ocr-review-page'>
    ...
    {mergePreview && <div className='reocr-dialog-backdrop' role='presentation' onMouseDown={() => setMergePreview(null)}><section className='reocr-dialog merge-preview-dialog' role='dialog' aria-modal='true' aria-labelledby='merge-preview-title' onMouseDown={event => event.stopPropagation()}><h2 id='merge-preview-title'>박스 병합 미리보기</h2><p>{mergePreview.elements.length}개 박스가 아래 범위와 텍스트로 합쳐집니다. 범위가 지나치게 크면 취소하고 같은 단락만 다시 선택하세요.</p><div className='merge-preview-box'><span style={{ left: `${mergePreview.box.x * 100}%`, top: `${mergePreview.box.y * 100}%`, width: `${mergePreview.box.width * 100}%`, height: `${mergePreview.box.height * 100}%` }}/></div><pre>{mergePreview.text}</pre><div className='reocr-dialog-actions'><button onClick={() => setMergePreview(null)}>취소</button><button className='primary' disabled={mergeMutation.isPending} onClick={() => mergeMutation.mutate(mergePreview.elements)}>{mergeMutation.isPending ? '병합 중...' : '이대로 병합'}</button></div></section></div>}
  </div>
}
